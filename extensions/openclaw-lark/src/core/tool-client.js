...
class ToolClient {
     ...
     * 仅执行用户权限预检，不实际调用 API。
     *
     * 适用于一个高阶动作内部会串联多个 API（例如先读主日历再创建事件）的场景。
     * 这样可以在真正发起第一个子调用前，一次性申请完整所需 scope，
     * 避免前置读接口先抛出较弱权限，导致 OAuth 只申请了部分 scope。
     */
    async ensureUserScopes(toolActions, options) {
        const actions = Array.isArray(toolActions) ? toolActions : [toolActions];
        const requiredScopes = (0, scope_manager_1.getRequiredScopesForActions)(actions);
        const tokenType = options?.as ?? 'user';
        const apiName = options?.apiName ?? actions[actions.length - 1] ?? 'unknown';
        ...
        if (tokenType === 'tenant') {
            return { requiredScopes, appScopeVerified };
        }
        ...
        if (appScopeVerified && stored.scope && requiredScopes.length > 0) {
            ...
            const missingUserScopes = requiredScopes.filter((scope) => !userGrantedScopes.has(scope));
        ...
        return { userOpenId, requiredScopes, appScopeVerified };
    }
    /**
