            ...
            try {
                ...
                switch (p.action) {
                    ...
                    case 'create': {
                        ...
                        const hasAttendees = (p.attendees?.length ?? 0) > 0 || Boolean(p.user_open_id);
                        if (hasAttendees) {
                            preflightActions.push('feishu_calendar_event_attendee.create');
                        }
                        await client.ensureUserScopes(preflightActions, {
                            as: 'user',
                            apiName: 'feishu_calendar_event.create',
                        });
