...
function coversRequestedScopes(existingScope, requestedScope) {
    const existing = new Set(normalizeScopeList(existingScope));
    return normalizeScopeList(requestedScope).every((scope) => existing.has(scope));
}
// ---------------------------------------------------------------------------
...
async function executeAuthorize(params) {
    ...
    if (pendingFlows.has(flowKey)) {
        ...
        const flowStillUsable = (oldFlow.expiresAtMs ?? 0) > Date.now();
        if (flowStillUsable && coversRequestedScopes(oldFlow.scope, effectiveScope)) {
            log.info(`reusing existing flow for user=${senderOpenId}, app=${appId}, cardId=${oldFlow.cardId}, scopes=[${normalizeScopeList(oldFlow.scope).join(', ')}]`);
            return json({
                success: true,
                message: '已有未完成的授权请求，请直接完成上一条最新授权卡中的授权。授权完成后重新执行之前的操作即可。',
                awaiting_authorization: true,
                reused_existing_authorization: true,
                scope: oldFlow.scope,
                expires_in_sec: Math.max(0, Math.ceil(((oldFlow.expiresAtMs ?? Date.now()) - Date.now()) / 1000)),
            });
        }
    ...
    const currentFlow = {
        ...
        expiresAtMs: Date.now() + deviceAuth.expiresIn * 1000,
    };
