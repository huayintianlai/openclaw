...
class FinalCertificateEditor:
    ...
    def draw_text_with_tracking(self, draw, text, position, font, color, tracking=0):
        ...
        if tracking == 0:
            # 无字间距，使用2层伪粗体
            self.draw_subtle_faux_bold(draw, text, (x, y), font, color)
        else:
            # 有字间距，逐字符绘制
            current_x = x
            for char in text:
                self.draw_subtle_faux_bold(draw, char, (current_x, y), font, color)
                # 计算字符宽度 + tracking
    ...
    def draw_subtle_faux_bold(self, draw, text, position, font, color):
        """
        极微妙的伪粗体 - 只用0.15px偏移，完美匹配原文
        """
        x, y = position
    ...
    def edit_certificate(self, psd_path, company_name, address, date, output_path=None, output_format='png'):
        ...
        if output_path is None:
            ...
            output_path = os.path.join(self.workspace, f"{base_name}_{safe_company_name}_final.{ext}")
...
def main():
    ...
    if len(sys.argv) >= 5:
        ...
        print("用法: python3 certificate_editor_v4.py <psd文件> <公司名> <地址> <日期> [格式] [输出路径]")
        sys.exit(1)
